...
function doubleTrouble(numbers, target) {
  ...
  const numbersTarget = [...numbers];

  for (let i = 0; i < numbersTarget.length - 1; i++) {
    if (numbersTarget[i] + numbersTarget[i + 1] === target) {
      numbersTarget.splice(i + 1, 1);
      i--;
    }
  }

  return numbersTarget;
}
