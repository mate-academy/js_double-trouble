...
function doubleTrouble(numbers, target) {
  const numbersMirror = [...numbers];
  for (let i = 1; i < numbersMirror.length; i++) {
    if (numbersMirror[i - 1] + numbersMirror[i] === target) {
      numbersMirror.splice(i, 1);
      i--;
    }
  }
  return numbersMirror;
}
