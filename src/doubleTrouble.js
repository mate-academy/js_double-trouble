...
function doubleTrouble(numbers, target) {
  const remainedNumbers = new Array(...numbers);
  for (let i = 1; i < remainedNumbers.length; i++) {
    if (remainedNumbers[i] + remainedNumbers[i - 1] === target) {
      remainedNumbers.splice(i, 1);
      i--;
    }
  }
